...
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # Separate the name_color column...
...
# It didn't quite work as expected as there's a space prepended before the second column's contents
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "___" # Puts "___" between the pieces (default is underscore)
        ) 
...
```{r}
# Your code here: 
some_dates <- c(05051988, 12101969, 01010001) # Creates a vector of dates w/o spaces
some_dates_converted <- mdy(some_dates) # Converts to Date, stores as some_dates_converted
class(some_dates_converted) # Check
...
gasol_hookshots %>%
  filter(year_new %in% c(2008, 2009)) %>%
  group_by(month_new) %>%
  count(month_new) 
